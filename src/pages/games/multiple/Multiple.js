...
const Multiple = () => {
  ...
  const generateProblems = () => {
    const newProblems = [];
    for (let i = 0; i < 10; i++) {
      const factor1 = Math.floor(Math.random() * 90) + 10; // 두 자리 수 (10~99)
      const factor2 = Math.floor(Math.random() * 90) + 10; // 두 자리 수 (10~99)
  ...
  const checkAnswer = () => {
    ...
    if (parseInt(userAnswer) === correctAnswer) {
      ...
      if (currentIndex < problems.length - 1) {
        ...
        const currentRecord = localStorage.getItem("record_multiple");
        if (
          !currentRecord ||
          parseFloat(totalTime) < parseFloat(currentRecord)
        ) {
          localStorage.setItem("record_multiple", totalTime);
        }
  ...
  return (
    ...
    >
      <PageTitle title="곱셈게임" />
      <h1 style={{ fontSize: "30px", fontWeight: "500" }}>곱셈</h1>
      <div
        style={{
          display: "flex",
